...
(function () {
  ...
  const UI_NOISE = /^(refund details|refund reason|partner refund value|order total|date ordered|order submitted|order timeline|dispute this refund|prepared incorrectly|missing|missing item|missing items|incorrect|incorrect item|incorrect items|category|quantity|qty|price|item|items|name|total|deliveroo|partner hub)$/i;
  ...
  function extractRefundedItems() {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (name && isValidItemName(name)) items.push({ name, reason: canonicalizeReason(lines[i]) });
    }
  ...
  function primaryRefundedItemName(items, refundReason) {
    const matched = itemsMatchingReason(items, refundReason);
  ...
  function buildDisputeFieldValues(items, refundReason) {
    ...
    const otherReason = [...new Set(matchedItems.map((item) => item.name))].join("\n");
  ...
  async function fillClaimsForm(payload, options = {}) {
    ...
    for (const [label, value, extras] of jobs) {
      ...
      if (label === L.reasonForDispute && payload.preparedIncorrectlyWhy) {
        await wait(options.fast || fastFillMode ? 120 : 320);
      }
